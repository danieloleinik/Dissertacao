...
\author{Daniel Oleinik e Mauro Fonseca
\thanks{Daniel Oleinik, CPGEI - Universidade Tecnológica Federal do Paraná (UTFPR) CEP 80.230-910 - Curitiba/PR - Brasil, e-mail: daniel.oleinik@outlook.com;
Mauro Fonseca, CPGEI - Universidade Tecnológica Federal do Paraná (UTFPR) CEP 80.230-910 - Curitiba/PR - Brasil, e-mail: maurofonseca@utfpr.edu.br.}}

\maketitle

\markboth{XXXVII SIMPÓSIO BRASILEIRO DE TELECOMUNICAÇÕES E PROCESSAMENTO DE SINAIS - SBrT2019, 29/09/2019--02/10/2019, PETRÓPOLIS, RJ}{XXXVII SIMPÓSIO BRASILEIRO DE TELECOMUNICAÇÕES E PROCESSAMENTO DE SINAIS - SBrT2019, 29/09/2019--02/10/2019, PETRÓPOLIS, RJ} 
...
\begin{chave}
\emph{Mesh}, fibra óptica, \emph{Smart Grid}, \emph{Smart Cities}, chaveador óptico.
\end{chave}
...
\begin{keywords}
Mesh, optic fiber, Smart Grid, Smart Cities, optical switch.
\end{keywords}
...
O SG necessita de grande flexibilidade para permitir a adição de serviços cada vez mais variados e com diferentes requisitos de comunicação, como monitoramento de sensores em tempo real e interação com atuadores e usuários finais \cite{Art_Aggarwal2010}. Sabe-se que uma rede IP deve atender, com elevado grau de satisfação, os requisitos de comunicação demandados \cite{Conf_Lobo2008}. Requisitos estes que, segundo \cite{Art_Aggarwal2010}, devem ser substancialmente consideráveis em valores de latência, justificando facilmente o uso de um meio físico como a fibra óptica assim como qualquer tentativa de redução de latência. 
%Os algoritmos de roteamento existentes já apresentam resultados ótimos com relação aos parâmetros relacionados à latência de rede, mas absolutamente todos eles trabalham com a ideia de otimização de uma topologia física, tendo desta forma suas performances limitadas à própria topologia onde são aplicados. 
...
%\subsection{PRIM}
%\label{subsec:Prim}
%O algoritmo PRIM é recorrentemente utilizado por muitos outros algoritmos para encontrar a árvore geradora mínima de um grafo completamente conectado. %Este algoritmo foi criado em 1930 por um matemático Tchecoslovaco chamado Vojt?ch Jarník, melhorado em 1957 por um matemático e cientista da computação americano chamado Robert Clay Prim (que acabou dando nome ao algoritmo) e posteriormente utilizado em 1959 por Edsger Dijkstra (responsável pela criação do Algoritmo de Dijkstra).
%
...
\begin{figure} [h]% normalmente utilizar [!t]
	\centering
	\includegraphics[width=0.8\columnwidth,natwidth=2\textwidth,natheight=300]{Switch_optico.jpg}
...
%\begin{figure} % normalmente utilizar [!t]
	%\centering
	%\includegraphics[width=3cm,natwidth=150,natheight=150]{Rede_Mesh.jpg}
...
\begin{figure} % normalmente utilizar [!t]
	\centering
	\includegraphics[width=2\columnwidth,natwidth=2\textwidth,natheight=400]{Rede_Estudo_PB.jpg}
...
\label{sec:proposta}
O presente artigo tem como proposta o protocolo BSN, cujo objetivo é a descoberta e estabelecimento de rotas de maneira automática e completamente desagregada do protocolo de roteamento utilizado na rede em questão (normalmente STP, mas potencialmente MSTP, RSTP, OSPF ou qualquer outro), sendo assim a rede em questão não é dependente do protocolo proposto não existindo requisito estrito de tempo para a execução do mesmo. O BSN pode ser utilizado em qualquer rede que contenha multi-caminhos ou redundâncias, como nas redes \emph{mesh} devido a maior possibilidade de recombinações. Para tanto, é necessário que a rede em questão possa ser representada por um grafo simples, ou seja, um grafo que não possui arestas paralelas ou pontas coincidentes (laços).  
...
\begin{figure} % normalmente utilizar [!t]
	\centering
	\includegraphics[width=5cm,natwidth=0.9\textwidth,natheight=200]{latency_link.png}
...
\begin{figure} % normalmente utilizar [!t]
	\centering
	\includegraphics[width=1.8\columnwidth,natwidth=2\textwidth,natheight=200]{Bypass_exemplo_PB.png}
...
\begin{figure}[t!]
		\centering
		\includegraphics[width=2cm,natwidth=0.3\textwidth,natheight=600]{Otimizada_Prim_PB.jpg} % <- formatos PNG, JPG e PDF
		\caption{Rede resultante após a utilização do algoritmo PRIM. O grafo representa a árvore mínima da rede e é possível notar que a profundidade máxima é de 15 saltos.}
		\label{fig_rede_otimizada_prim}
		\end{figure}
	...
	\begin{figure}[t]
		\centering
		\includegraphics[width=2cm,natwidth=0.3\textwidth,natheight=450]{Otimizada_STP_PB.jpg} % <- formatos PNG, JPG e PDF
	\caption{Rede resultante após a utilização do STP considerando o nó raiz como sendo o vértice ``E''. Percebe-se que a profundidade máxima da rede é de 11 saltos.}
	\label{fig_rede_otimizada_STP}
\end{figure}
...
\begin{figure} % normalmente utilizar [!t]
	\centering
	\includegraphics[width=4cm,natwidth=0.8\textwidth,natheight=450]{Otimizada_BSN_PB.jpg}
...
O objetivo principal do BSN é o de fornecer configurações de topologia de rede ópticas que possibilitem o estabelecimento de comunicações com valores de latência otimizados. Como descrito anteriorimente, cada equipamento existente em um ramo de comunicação é uma fonte de inserção de latência na rede, sendo assim pode-se afirmar que cada \emph{hop} adicional em uma comunicação pode ser traduzido como uma inserção de latência. Logo, ao encurtar os ramos do grafo minimiza-se o valor de latência da rede já que a quantidade de banda ocupada não é problema para a tecnologia atual.

A partir dos resultados apresentados na Seção \ref{sec:resultados} pode-se notar uma sensível mudança no parâmetro de profundidade máxima do grafo. Observando-se as Figuras \ref{fig_rede_otimizada_prim}, \ref{fig_rede_otimizada_STP} e \ref{fig_rede_otimizada_BSN}) nota-se uma variação na largura do mesmo em relação a profundidade. Ou seja, tem-se um grafo com uma média de \emph{hops} menor para cada destino, resultando desta forma em menores valores de latência para a rede como um todo.
...
Para fins de comparação pode-se analisar a latência de transmissão considerando o meio fibra-óptica através da velocidade média de propagação da luz no núcleo da fibra, de onde pode-se tirar que a latência neste meio é de aproximadamente 5 \textmu s/km. Logo, considerando que cada enlace entre dois pontos na rede da Figura \ref{fig_rede_estudo} possua, em média, 1km de extensão e que o tempo de processamento em cada um dos nós (desconsiderando o aumento de tempo no caso de excesso de processamento em determinados nós) é de 1ms na média (já incluindo os tempos das conversões de meio necessárias), gera-se A Figura \ref{fig_tab_percentual}.
%a Tabela \ref{tab_percentual} que mostra a melhoria nos parâmetros avaliados entre os três protocolos.
...
\bibliography{SMMC2018}
\bibitem{Art_Wojdak} W. Wojdak, \textit{Rapid spanning tree protocol: A new solution from an old technology}. Performance Technologies,
2003.
\bibitem{Art_Krishnan} Y. N. Krishnan e C. Bhagwat e A. P. Utpat, \textit{Optimizing Spanning Tree Protocol using Port Channel}. IEE, 2014.

\bibitem{Art_Aggarwal2010} A. Aggarwal e S. Kunta e P. K. Verma, \textit{A Proposed Communications Infrastructure for the Smart Grid}. IEE, 2010.
\bibitem{Conf_Lobo2008} F. Lobo e A. Cabello e A. Lopez e D. Mora e R. Mora, \textit{Distribution Network as Communication system}. CIRED Seminar 2008: Smart Grids for distribution, 2008.
\bibitem{Art_Amin2006} M. Amin, \textit{Toward a Self-healing Energy Infrastructure}. IEE, 2006.
\bibitem{Conf_Sood2009} V. Sood e D. Fischer e M. Eklund e T. Brown, \textit{Developing a Communication Infrastructure for the Smart Grid}. IEEE Electrical Power \& Energy Conference, 2009.
\bibitem{Art_Gungor2013} V. Gungor e D. Sahin e T. Kocak e S. Ergut e C. Buccella e C. Cecati e G. Hancke, \textit{A Survey on Smart Grid Potential Applications and Communication Requirements}. IEEE TRANSACTIONS ON INDUSTRIAL INFORMATICS, 2013.
\bibitem{inbook_data_cabling} \textit{Data Cabling System - Introdução à Área de Cabeamento Estruturado de Redes, página 64}. FURUKAWA, 2015.
\bibitem{Accelink2014} \textit{OSW-22BUM-135-09-1-N-S-SC/UPC ACCELINK Product Specification}. Accelink, 2014.
\bibitem{Art_Coffey} C. Joseph, \textit{Latency in Optical Fiber Systems}. Disponível em: https://www.commscope.com/Docs/Latency\_in\_optical\_fiber\_systems\_WP-111432-EN.pdf, 2017.
\bibitem{Mag_Bouhafs2012} F. Bouhafs e M. Mackay e M. Merabti, \textit{Links to the Future}. IEE, 2012.
